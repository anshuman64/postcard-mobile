...
import { COLORS, getScaledHeight } from '../../utilities/style_utility';
...
class Medium extends React.PureComponent {
  ...
  componentDidMount() {
    if (this.props.data.contentType.startsWith('text/') && this.props.data.images) {
      ...
      this._setMediumDimensions(this.props.data.url, 'video');
    }
  }
  ...
  _setMediumDimensions = (url, previewType) => {
    RN.Image.getSize(url, (width, height) => {
      ...
      let scaledHeight = getScaledHeight({ width: width, height: height }, scaledWidth);
    ...
    return (
      <RN.View style={[styles.mediumContainer, { width: this.state.scaledDimensions.width, height: this.state.scaledDimensions.height }]}>
        ...
        <VideoPlayer
          ...
          videoWidth={this.state.mediumDimensions.width}
          videoHeight={this.state.mediumDimensions.height}
          autoPlay={true}
          defaultMuted={true}
